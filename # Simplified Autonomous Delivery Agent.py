...
class Courier:
    ...
    def run_delivery(self, start, end, algo, animate=True):
        self.x, self.y = start
        print(f"\nLubhani starting delivery {start} -> {end} in {self.city.name} using {algo.upper()}")
        if algo == "bfs":
        ...
        if (self.x, self.y) == end:
            self.history.append(f"Lubhani delivered in {self.city.name}! Cost={total_cost}")
            self.done += 1
...
def menu():
    ...
    print("LUBHANI DELIVERY SIMULATOR")
    print("=" * 40)
    while True:
        print("\nSelect City:")
        print("1. Bhopal")
        ...
        if choice == "5":
            print("Bye from Lubhani Simulator!")
            break
        if choice not in city_options:
            print("Invalid selection.")
            continue
        lubhani_city = city_options[choice]()
        lubhani_bot = Courier(lubhani_city)
        while True:
            print(f"\n You are in {lubhani_city.name}!")
            ...
            if sub == "1":
                ...
                start, end = lubhani_city.deliveries[0]
                ...
                cost, nodes, ok = lubhani_bot.run_delivery(start, end, algo, animate)
                print(f"Result: Cost={cost}, Explored={nodes}, Success={ok}")
                input("Press Enter...")
            elif sub == "3":
                start, end = lubhani_city.deliveries[0]
                for m in ["bfs", "ucs", "astar", "local"]:
                    lubhani_bot.clock, lubhani_bot.fuel = 0, 100
                    cost, nodes, ok = lubhani_bot.run_delivery(start, end, m, animate=False)
                ...
                for log in lubhani_bot.history[-10:]:
                    print(log)
                input("Press Enter...")
            elif sub == "6":
                break
